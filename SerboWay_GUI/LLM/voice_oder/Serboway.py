...
def process_order_node(state: OrderState):
    ...
    if not ingredients:
        return {
            "messages": [
                AIMessage("재료를 선택해주세요 (햄/치즈/양상추), 최소 1개, 최대 3개까지 고를 수 있어요.")
            ],
            "order": order,
        }

    # 2) 1개 이상 선택했지만 아직 '완료'를 말하지 않은 경우
    if not done:
        ...
        return {
            "messages": [
                AIMessage(
                    f"지금까지 선택한 재료: {sel}.\n"
                    "추가할 재료가 있으면 말씀해주세요. "
                    "준비가 되셨으면 '완료'라고 말해주세요."
                )
            ],
            "order": order,
        }

    # 3) '완료' 키워드 입력된 경우 → 주문 확정 요청
    sel = ", ".join(ingredients)
    return {
        "messages": [
