...
# --- 데이터 구조 정의 ---
MENU_DATA = {
    "불고기 샌드위치": {"price": 6500, "description": "부드러운 불고기가 들어간 샌드위치"},
    "새우 샌드위치": {"price": 6200, "description": "신선한 새우가 들어간 샌드위치"},
    "베이컨 샌드위치": {"price": 6000, "description": "바삭한 베이컨이 들어간 샌드위치"},
}
...
@tool
def update_order(tool_input: dict) -> str:
    """
    tool_input: {"menu": ..., "sauce": ..., "vegetable": ..., "cheese": ...}
    """
    order_state = st.session_state.order_state
    menu = tool_input.get("menu")
    sauce = tool_input.get("sauce")
    vegetable = tool_input.get("vegetable")
    cheese = tool_input.get("cheese")
    ...
    if sauce and sauce in SAUCE_DATA:
        ...
        order_state.step = "vegetable" if order_state.step == "sauce" else order_state.step
    if vegetable and vegetable in VEGETABLE_DATA:
        order_state.vegetable = vegetable
        order_state.step = "cheese" if order_state.step == "vegetable" else order_state.step
    if cheese and cheese in CHEESE_DATA:
        order_state.cheese = cheese
        order_state.step = "confirm" if order_state.step == "cheese" else order_state.step
...
@tool
def confirm_order(tool_input: bool) -> str:
    """주문을 확정하거나 취소합니다."""
    order_state = st.session_state.order_state
    if tool_input:
...
def initialize_agent():
    ...
    update_order는 {"menu": ..., "sauce": ..., "vegetable": ..., "cheese": ...} 형식의 dict를 입력받습니다.
    """
...
def main():
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            AIMessage(content="어서오세요! 서보웨이에 오신 것을 환영합니다. 무엇을 도와드릴까요?")
        ]
