...
class ArucoNavigator(Node):
    def __init__(self, calib):
        ...
        # pose estimation marker length (cm)
    ...
    def timer_cb(self):
        ...
        # 4) Pose estimation for marker 5 & draw front arrow
        ...
        if ids is not None and self.cached_H is not None and 5 in ids.flatten():
            ...
            # world front coord via homography
            wx_f, wy_f = pixel_to_world(self.cached_H, fx, fy)

            cv2.putText(img, f"Robo@({int(wx_r)},{int(wy_r)})",
                        (10,30), cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,255,0),2)

        # 5) Driving control using world-front vs target
        twist = Twist()
        if (ids is not None and self.cached_H is not None
            and wx_r is not None and wx_f is not None
            and self.wp_idx < len(self.waypoints)):
            wp_id = self.waypoints[self.wp_idx]
            if wp_id in ids.flatten():
                # waypoint world coord
                i_w    = list(ids.flatten()).index(wp_id)
                cw     = corners[i_w][0]
                px_w = float(cw[:,0].mean());  py_w = float(cw[:,1].mean())
                wx_w, wy_w = pixel_to_world(self.cached_H, px_w, py_w)
                cv2.putText(img, f"WP{wp_id}@({int(wx_w)},{int(wy_w)})",
                            (10,90), cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,255,0),2)

                # compute front and target vectors in world frame
                front_vec  = np.array([wx_f - wx_r, wy_f - wy_r])
                target_vec = np.array([wx_w - wx_r, wy_w - wy_r])
                dist_err   = np.hypot(target_vec[0], target_vec[1])
                ...
                # Proportional control for smooth path
                K_ang = 0.005   # 실험적으로 조정
                K_lin = 0.002   # 실험적으로 조정

                # angular velocity ~ err
                ang = K_ang * err
                twist.angular.z = max(-0.3, min(0.3, ang))

                # linear velocity ~ dist_err, attenuated by heading error
                speed_factor = max(0.0, math.cos(math.radians(err)))
                lin = K_lin * dist_err * speed_factor
                twist.linear.x = max(0.0, min(0.2, lin))
