...
class ArucoNavCommander(Node):
    ...
    def vision_cb(self):
        ...
        if self.cached_H is not None:
            ...
            # ← 이 아래에 로봇↔목표 선분 그리기 삽입
        # 로봇↔목표 선분 그리기 (매 프레임이 아닌, 발행 시점 위치 사용)
            if self.prev_pose_for_line and self.active_target:
                rx, ry = self.prev_pose_for_line
                rpx = int(rx * PPM)
                rpy = int(MAP_HEIGHT_CM * PPM - ry * PPM)
                tx, ty = self.active_target
                ntx = int(tx * PPM)
                nty = int(MAP_HEIGHT_CM * PPM - ty * PPM)
                cv2.line(tf_map, (rpx, rpy), (ntx, nty), (255, 0, 0), 2)


            cv2.imshow("tf_map", tf_map)

        # active_target이 설정돼 있을 때 로봇과 목표를 연결하는 선
        if self.cached_H is not None and self.active_target is not None:
            # 로봇 픽셀 좌표 계산
            rx, ry = self.current_pose
            rpx, rpy = int(rx * PPM), int(MAP_HEIGHT_CM * PPM - ry * PPM)
            # 다음 목표 픽셀 좌표 계산
            tx, ty = self.active_target
            ntx, nty = int(tx * PPM), int(MAP_HEIGHT_CM * PPM - ty * PPM)
            # 파란 선 그리기
            cv2.line(
                tf_map,
                (rpx, rpy),
