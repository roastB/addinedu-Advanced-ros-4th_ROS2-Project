...
def build_graph():
    node_coords = {
        ...
        'mid': id_to_coord[98], 'Home': id_to_coord[99],
    }
    adj = {
        'P4':['P1','P3'], 'P3':['P2','P4'], 'P2':['P3','mid'], 'P1':['P3','mid'],
        'mid':['P1','P2'], 'Home':['mid'], 'Pick':['P2'], 'Drop':['P4'],
...
class ArucoNavCommander(Node):
    def __init__(self):
        ...
        self.get_logger().info("0~9 키 입력 → A* 경로 생성 및 전송")
    ...
    def vision_cb(self):
        ...
        if chr(key).isdigit():
            ...
            if pid in self.key_to_dest_id and self.current_pose is not None:
                ...
                start = min(
                    self.node_coords,
                    key=lambda n: math.hypot(self.current_pose[0]-self.node_coords[n][0],
                ...
                # fill path_queue with world coords
