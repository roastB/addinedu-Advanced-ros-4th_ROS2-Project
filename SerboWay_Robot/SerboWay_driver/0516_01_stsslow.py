...
class DriveNode(Node):
    def __init__(self):
        ...
        # 상태 머신 변수: IDLE, ROTATING, PAUSE, DRIVING, STOP_OBS, ROTATING_OBS, PAUSE_OBS, STOPPED
        self.state        = 'IDLE'
        ...
        if self.state == 'ROTATING_OBS':
            ...
            if self.accum_time < self.turn_interval:
                ...
                if abs(delta) >= math.pi/2:
                    self.state = 'STOPPED'
                    self.get_logger().info("▷ 90° ROTATING_OBS done — STOPPED")
                    self.cmd_pub.publish(Twist())
                else:
                    self.state = 'PAUSE_OBS'
                return

        # PAUSE_OBS: mini-pause 후 다시 ROTATING_OBS
