...
class ArucoNavCommander(Node):
    ...
    def vision_cb(self):
        ...
        # 키 입력 처리 (로봇 선택 → 경로 생성 → 종료)
        key = cv2.waitKey(1) & 0xFF

        # 1) 로봇 선택: '5', '6', '7' 키
        if key in (ord('5'), ord('6'), ord('7')):
            self.selected_robot = int(chr(key))
            self.get_logger().info(f"로봇 {self.selected_robot} 선택됨")
            return  # 선택만 하고 경로 생성 로직은 실행하지 않음

        # 2) 목적지 선택: 숫자키(0,1,2,3,8,9)
        #    self.selected_robot 이 5,6,7 중 하나일 때만 처리
        if chr(key).isdigit() and self.selected_robot in self.robot_ids:
            pid = int(chr(key))
            # 키에 매핑된 목적지가 있을 때
            if pid in self.key_to_dest_id and self.current_pose[self.selected_robot] is not None:
                rid = self.selected_robot

                # --- 경로 생성 (A* 탐색 + 필터링) ---
                # 1) 현재 위치에서 가장 가까운 코너 노드 찾기
                start = min(
                ...
                # 5) 경로 필터링 (코너·키 노드만 남기기)
                corner = {'P1','P2','P3','P4'}
                keyn   = {'Home','Pick','Drop','T1','T2','T3'}
                filtered = []
                if best_path[0] not in corner|keyn:
