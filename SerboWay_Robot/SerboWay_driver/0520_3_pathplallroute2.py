    ...
    def vision_cb(self):
        ...
        if chr(key).isdigit():
            ...
            if pid in self.key_to_dest_id and self.current_pose is not None:
                # pick start among all except T1/T2/T3
                valid_nodes = [
                ...
                # --- Filter: include transitions around allowed nodes ---
                ...
                filtered = []
                for i, node in enumerate(best_path[1:], start=1):
                    prev = best_path[i-1]
                    if prev in allowed or node in allowed:
