    ...
    def vision_cb(self):
        ...
        if self.cached_H is not None:
            ...
            for x, y in self.path_queue:
                px_, py_ = int(x*PPM), int(int(MAP_HEIGHT_CM*PPM) - y*PPM)
                cv2.circle(tf_map, (px_, py_), 6, (0,255,0), -1)
            if self.active_target:
                tx, ty = self.active_target
                px_, py_ = int(tx*PPM), int(int(MAP_HEIGHT_CM*PPM) - ty*PPM)
                cv2.circle(tf_map, (px_, py_), 8, (0,0,255), -1)
        ...
        if chr(key).isdigit():
            ...
            if pid in self.key_to_dest_id and self.current_pose is not None:
                # Exclude T1/T2/T3 when picking start node
                valid_nodes = [n for n in self.node_coords if n not in ('T1','T2','T3')]
                start = min(
                    valid_nodes,
                ...
                # Fill path_queue
                self.path_queue = [self.node_coords[n] for n in best_path[1:]]
        elif key == ord('q'):
            rclpy.shutdown()
