...
class DriveNode(Node):
    def __init__(self):
        ...
        self.off_tol           = 0.02     # m, 측면 ±2cm
        self.off_line_tol      = 0.03     # m, 선 벗어남 기준 (추가)
        self.adjust_drive_t    = 0.1      # s, 미니회전 후 전진 시간

        self.mini_deg         = 20.0     # deg
        self.mini_rad         = math.radians(self.mini_deg)

        self.state        = 'IDLE'
        self.big_forward_t     = 0.3    # 대수정 시 처음 전진 시간
        self.rotation_dir = 0.0
        self.accum_time   = 0.0
        self.prev_time    = time.time()
        self.prev_pose_for_line = None    # ← 여기 추가
        # 5) 제어 루프 타이머 (200Hz)
    ...
    def control_loop(self):
        ...
        if self.state == 'ROTATE90':
           
            heading = math.atan2(self.wy_f - self.wy_r, self.wx_f - self.wx_r)
            delta   = (heading - self._obs_heading0 + math.pi) % (2*math.pi) - math.pi
            abs_delta = abs(delta)
            # 1) 아직 90°−mini_deg (rad) 이전: 연속 회전
            if abs_delta >= (math.pi/2 - self.mini_rad):
                # 마지막 mini_deg 구간 진입 → 미니회전
                if abs_delta > math.pi/2:
                    # 90° 넘으면 완료
                    self.state = 'MEASURE_SIDE'
                    self.accum_time = 0.0
                else:
                    # mini bang–bang
                    if self.accum_time < self.turn_interval:
                ...
                # 90°−mini 구간 이전은 연속 회전
                twist.angular.z = self.rotation_dir * self.fixed_omega
                self.cmd_pub.publish(twist)
            return
        
        if self.state == 'MEASURE_SIDE':
            if not math.isnan(self.d_f) and self.d_f < self.obs_detect_thresh:
                self.state, self.accum_time = 'CONFIRM_OBS', 0.0
                return            
            side = self.d_r if self.rotation_dir>0 else self.d_l
            self._side_ref = side
            self.state = 'DETECT_OFF_LINE'
            return
        
        if self.state == 'DETECT_OFF_LINE':
            if not math.isnan(self.d_f) and self.d_f < self.obs_detect_thresh:
                self.state, self.accum_time = 'CONFIRM_OBS', 0.0
                return
            # 측면 거리 차이 계산
            side = self.d_r if self.rotation_dir>0 else self.d_l
            diff = side - self._side_ref

            # 1) 대수정 필요: |diff| > 0.20
            if abs(diff) > 0.20:
                self.state = 'BIG_ADJUST_START'

            # 2) 보정 완료: |diff| ≤ off_tol
            elif abs(diff) <= self.off_tol:
                self.state = 'RECOVER_LINE'

            # 3) 중간 오차: off_tol < |diff| ≤ 0.20
            else:
                self.state = 'ADJUST_SIDE'

            return
        
        if self.state == 'ADJUST_SIDE':
            if not math.isnan(self.d_f) and self.d_f < self.obs_detect_thresh:
                self.state, self.accum_time = 'CONFIRM_OBS', 0.0
                return           
            side = self.d_r if self.rotation_dir>0 else self.d_l
            diff = side - self._side_ref
            if abs(diff) <= self.off_tol:
                self.state = 'RECOVER_LINE'
            else:
                # (a) 미니회전 5도 정도: turn_interval 조절로 5도 회전
                twist.angular.z = self.rotation_dir * self.fixed_omega
                self.cmd_pub.publish(twist)
                ...
                # (c) 전진
                twist = Twist(); twist.linear.x = self.drive_speed
                self.cmd_pub.publish(twist)
                time.sleep(self.adjust_drive_t)
            return
        
        # ── 대수정 플로우 시작 ─────────────────────────────
        if self.state == 'BIG_ADJUST_START':
            if not math.isnan(self.d_f) and self.d_f < self.obs_detect_thresh:
                self.state, self.accum_time = 'CONFIRM_OBS', 0.0
                return
            # 1) 전진 0.3s
            twist = Twist(); twist.linear.x = self.drive_speed
            self.cmd_pub.publish(twist)
            time.sleep(self.big_forward_t)
            # 다음은 90도 회전 단계로
            # 시작 헤딩 기록
            self._obs_heading0 = math.atan2(self.wy_f - self.wy_r,
                                            self.wx_f - self.wx_r)
            self.state = 'BIG_ROTATE90'
            return

        if self.state == 'BIG_ROTATE90':
            # 2) 90도 회전 (마지막 20도만 미니회전)
            heading = math.atan2(self.wy_f - self.wy_r,
                                  self.wx_f - self.wx_r)
            delta   = (heading - self._obs_heading0 + math.pi) % (2*math.pi) - math.pi
            abs_delta = abs(delta)
            # 처음 (π/2 - mini_rad) 이전: 연속 회전
            if abs_delta < math.pi/2 - self.mini_rad:
                twist.angular.z = self.rotation_dir * self.fixed_omega
                self.cmd_pub.publish(twist)
            else:
                # 마지막 mini_rad 구간: bang–bang
                if self.accum_time < self.turn_interval:
                    twist.angular.z = self.rotation_dir * self.fixed_omega
                elif self.accum_time < self.turn_interval + self.pause_interval:
                    twist.angular.z = 0.0
            ...
            # 90° 넘으면 측면 재측정 플로우로
            if abs_delta >= math.pi/2:
                self.state = 'BIG_FORWARD_TILL_MID'
                self.accum_time = 0.0
            return

        if self.state == 'BIG_FORWARD_TILL_MID':
            if not math.isnan(self.d_f) and self.d_f < self.obs_detect_thresh:
                self.state, self.accum_time = 'CONFIRM_OBS', 0.0
                return
            # 3) 측면 거리가 off_tol<|diff|<=0.20 될 때까지 전진
            side = self.d_r if self.rotation_dir>0 else self.d_l
            if self.off_tol < abs(side - self._side_ref) <= 0.20:
                # 4) 기준값 업데이트
                self._side_ref = side
                # 5) 기존 ADJUST_SIDE 로 복귀
                self.state = 'ADJUST_SIDE'
            else:
                twist = Twist(); twist.linear.x = self.drive_speed
                self.cmd_pub.publish(twist)
        ...
        if self.state == 'RECOVER_LINE':
            ...
            # 2) 로봇 위치 → line(prev_pose_for_line, active_target) 거리 계산
            if self.prev_pose_for_line and self.active_target and self.current_pose:
                # 점 P = current_pose, 선분 AB = (prev_pose_for_line → active_target)
                (x0, y0) = self.current_pose
                (x1, y1) = self.prev_pose_for_line
                (x2, y2) = self.active_target
                # 점-선분 거리 공식
