...
class ArucoNavCommander(Node):
    def __init__(self):
        ...
        # ─── 여기에 Robot별 Home 좌표 매핑 ───
        # 5 → Home(id 99), 6 → Home2(id 98), 7 → Home3(id 97)
        self.robot_home = {
            5: id_to_coord[99],   # (92, 15)
            6: id_to_coord[98],   # (98, 85)
            7: id_to_coord[97],   # (10, 97)
        }
        # ────────────────────────────────────
        self.robot_home_id = {
            5: 99,
    ...
    def vision_cb(self):
        ...
        if chr(key).isdigit() and self.selected_robot in self.robot_ids:
            ...
            # (선택사항) 기존 대기 중이던 경로 클리어
            self.path_queue[rid].clear()
            # 2-1) 0번(Home)도 A* 로직으로 처리
            if pid == 0:
                goal_id = self.robot_home_id[rid]    # __init__에서 {5:99,6:98,7:97}로 정의
            else:
